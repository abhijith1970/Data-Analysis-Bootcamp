...
SELECT min(totalamount) [MInumum amount]from dbo.Sales

SELECT max(saledate) [Maximum sales date] FROM dbo.Sales--gives based on latest date

SELECT max(paymentmethod) [Maximum payment method] FROM dbo.Sales--gives based on alphabetical order ik dumb asf


SELECT productID,max(quantity) [Max Quantity] FROM dbo.Sales
group by ProductID;

SELECT Saledate,max(totalamount) [Maximum amount on the each dates ] FROM dbo.Sales
group by SaleDate